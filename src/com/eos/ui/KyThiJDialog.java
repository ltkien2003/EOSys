...
//        kt.setMaKyThi(kt.getMaKyThi());
    ...
    void insert() {
        ...
        boolean kiemTraTGMoDe = true;
        boolean kiemTraTGDongDe = true;
        try {
        ...
        if (!Auth.isAdmin()) {
            ...
            MsgBox.alert(this, "TG mở đề không được bỏ trống");
            txtTGMoDe.requestFocus();
        } else if (tgDongDe.equals("")) {
            MsgBox.alert(this, "TG đống đề không được bỏ trống");
            txtTGDongDe.requestFocus();
        } else if (kiemTraTGMoDe == false || !tgMoDe.matches(kiemTraDinhDangTGDongMoDe)) {
            MsgBox.alert(this, "TG mở đề chưa đúng định dạng dd/MM/yyyy hh:mm:ss");
            txtTGMoDe.requestFocus();
        } else if (kiemTraTGDongDe == false || !tgDongDe.matches(kiemTraDinhDangTGDongMoDe)) {
            MsgBox.alert(this, "TG đống đề chưa đúng định dạng dd/MM/yyyy hh:mm:ss");
            txtTGMoDe.requestFocus();
        } else {
            try {
                KyThi kt = getForm();
    ...
    void fillTable() {
        ...
        try {
            KhoaHoc khoahoc = (KhoaHoc) cboKhoaHoc.getSelectedItem();
            Chuong chuong = (Chuong) cboTenChuong.getSelectedItem();
            List<KyThi> list = ktdao.selectByKhoaHoc(khoahoc.getMaKH(), chuong.getMaChuong());
            for (KyThi kt : list) {
                Object[] row = {
                    kt.getMaKyThi(),
                    kt.getTenKT(),
...
//            System.out.println("you're bingchilling ");
        }
    }
